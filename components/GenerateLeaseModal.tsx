          ...
          <div className="max-w-[210mm] mx-auto bg-white shadow-xl p-16 min-h-[297mm] text-slate-800 font-serif leading-relaxed text-sm">
            ...
            <div className="space-y-8">
              ...
              <section>
                ...
                {/* Fixed: Use rentAmount as defined in types.ts */}
                <p>The Monthly Rent is <strong>KES {tenant.rentAmount.toLocaleString()}</strong>, payable in advance on the <strong>{tenant.dueDate}th day</strong> of each calendar month via M-Pesa or Bank Transfer.</p>
                {/* Fixed: Use leaseEnd property instead of leaseExpiryDate */}
                <p className="mt-2">The lease term shall commence on <strong>{today}</strong> and expire on <strong>{new Date(tenant.leaseEnd).toLocaleDateString()}</strong>.</p>
              </section>
